...
export const BottomNav: React.FC<BottomNavProps> = ({ currentScreen, onNavigate, isDark }) => {
  ...
  const NavItem = ({ screen, icon, label, filled = false }: { screen: ScreenName, icon: string, label: string, filled?: boolean }) => {
    ...
    return (
      <button 
        ...
        class={`flex flex-col items-center justify-center w-16 h-12 gap-1 transition-colors relative ${isActive ? activeClass : inactiveClass}`}
      >
  ...
  return (
    <nav className={`fixed bottom-0 left-0 w-full z-50 backdrop-blur-xl border-t pb-safe pt-2 ${navClass}`}>
      <div className="flex justify-around items-center px-2 pb-5 h-14">
        <NavItem screen="feed" icon="home" label="Home" filled={currentScreen === 'feed'} />
        <NavItem screen="events" icon="explore" label="Events" />
        ...
        <NavItem screen="arena" icon="trophy" label="Arena" />
